...
@Component({
    ...
    styleUrl: './audit-add.component.scss'
})
export class AuditAddComponent {
    ...
    ngOnInit() {
        ...
        const form = this.moduleService.getFormById('SFA', 'Branding', this.FORMID.ID['AuditForm']);
        if (subModule) {
    ...
    onFieldChange(value: any, field: any) {
        ...
        if (field.type === this.fieldTypes.UPLOAD) {
            ...
            if(field.name === 'competitor_image'){
                this.compImages = value;
            }
        }
    ...
    onSubmit() {
        ...
        if (this.myForm.valid) {
            ...
            this.pondFiles = [...this.compImages, ...this.images];
            // Only using the create API now
            this.api.post(result.primaryFields, 'brand-audit/create-brand-audit').subscribe(response => {

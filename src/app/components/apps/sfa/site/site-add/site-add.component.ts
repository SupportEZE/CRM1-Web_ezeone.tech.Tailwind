...
export class SiteAddComponent {
    ...
    constructor(public moduleService: ModuleService, public api: ApiService, public util: UtilService, private formValidation: FormValidationService, private fb: FormBuilder, public dialog: MatDialog, public route: ActivatedRoute, private router: Router, private logService: LogService, private toastr: ToastrServices, private authService: AuthService, public spaceRemove:RemoveSpaceService,public uploadService: UploadFileService, public comanFuncation: ComanFuncationService){}
    ...
    onFieldChange(value: any, field: any) {
        ...
        if (field.name === "site_code" && value.length >= 3) {
        ...
        if (field.name === "pincode" && value.length === 6 && this.orgData?.org?.postal_auto_fetch === 1) {// Postal Pincode //
    ...
    onSubmit() {
        ...
        if (this.myForm.valid) {
            ...
            const functionName = isEditMode ? 'sites/update' : 'sites/create';
            
            this.api[httpMethod](result.primaryFields, functionName).subscribe(result => {
                if (result['statusCode'] === 200) {
                    if (this.UploadFiles.length> 0){
                        this.api.disabled = true;
                        this.uploadService.uploadFile(result['data']['inserted_id'], 'sites',  this.UploadFiles, 'Site Images', this.submodule, '/apps/sfa/site')
                    }
                    else
                    {
                        this.api.disabled = false;
                        this.comanFuncation.setHighLight('userList', 'add', '', {}, 1);
                        this.router.navigate([!isEditMode ? '/apps/sfa/site' : `/apps/sfa/site/site-detail/${this.DetailId}`]);
    ...
    checkDuplicate(api:string, code:string){
        this.api.post({'site_code':code}, api).subscribe(result => {
            if(result === false){

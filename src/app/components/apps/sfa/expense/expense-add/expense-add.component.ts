...
export class ExpenseAddComponent {
  ...
  ngOnInit(){
    ...
    const tableId = this.moduleService.getTableById('SFA', 'Expense', this.FORMID.ID['ExpenseTableData']);
    if (subModule) {
  ...
  getFormData(){
   ...
   initializeForm(){
     ...
     this.formFields.forEach(field => {
       let validators = this.formValidation.getValidators(field) || [];
       
       if(field.min_date === true){
         if(field.min_period === 'today'){
           field.min_period ='last2days'
           field.min_period = new Date();
           field.min_period.setDate(field.min_period.getDate() - 2);
         }
         validators.push(DateValidators.minDate(new Date(field.min_period)));
       }
       if(field.max_date === true){
         if(field.max_period === 'today'){
           field.max_period = new Date();
           field.max_period.setDate(field.max_period.getDate() + 2);
         }
         validators.push(DateValidators.maxDate(new Date(field.max_period)));
       }
       ...
       this.myForm.setValidators(DateValidators.startToEndDateValidator('start_date', 'end_date'));
  ...
  getOptions(value:any){
    this.api.post({'user_id' : value},'expense/expense-config').subscribe(result => {
      if(result['statusCode'] === 200){
        for (let i = 0; i < this.formFields.length; i++) {
          if(this.formFields[i]['name'] === 'expense_type'){
            this.formFields[i]['options'] = result['data'];
          }
        }
        for (let i = 0; i < this.originalFormFields.length; i++) {
          if(this.originalFormFields[i]['name'] === 'expense_type'){
  ...
  handleSuccess(data: []) {
    ...
    // Posting the form data for 'Expense-create'
    this.api.post(result.primaryFields, 'expense/create').subscribe(
      (response) => {
        if (response['statusCode'] === 200) {
          this.api.disabled = false;
          this.router.navigate(['/apps/sfa/expense-list']);
          this.toastr.success(response['message'], '', 'toast-top-right');
        }
      },
      (error) => {
        this.api.disabled = false;
        this.toastr.error('Error creating Expense request', '', 'toast-top-right');

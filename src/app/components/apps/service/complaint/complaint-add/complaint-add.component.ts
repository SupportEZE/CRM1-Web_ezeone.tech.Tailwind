    ...
    constructor(
      ...
      initializeForm(){
        ...
        this.formFields.forEach(field => {
          if(field.api_path){
            this.getOptions(field.name, field.name, field.api_path, field.api_payload, field.api_payload_key)
          }
      ...
      onFieldChange(value: any, field: any) {
        ...
        if (field.name === "pincode" && value.length === 6 && this.orgData.postal_auto_fetch === 1) {// Postal Pincode //
      ...
      onSubmit() {
        ...
        if (this.myForm.valid) {
          ...
          const functionName = isEditMode ? 'complaint/update' : 'complaint/create';
          
          this.api[httpMethod](result.primaryFields, functionName).subscribe(result => {
            if (result['statusCode'] === 200) {
              if (this.UploadFiles.length> 0){
                this.api.disabled = true;
                this.uploadService.uploadFile(result['data']['inserted_id'], 'complaint',  this.UploadFiles, 'Complaint Images', this.submodule, '/apps/service/complaint')
              }
              else
              {
                this.api.disabled = false;
                this.router.navigate([!isEditMode ? '/apps/service/complaint' : `/apps/service/complaint/complaint-detail/${this.DetailId}`]);
